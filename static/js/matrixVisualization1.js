function MatrixVisualization() {
  ...
  var _colorScale = d3.scale
    .linear()
    .domain([0, 1])
    .range([
      AugmentedNodeTrix._colorScaleStart,
  ...
  var _matrix,
    ...
    _dissMatrix,
    _eigenMatrix,
    _unnormalizedAdjacencyMatrix,
    _matrixSize,
    _matrixBeforeClustering,
    _matrixRowSize,
    _matrixColumnSize,
    _labelsData,
    _rectMatrixAfterClustering,
    _axisPositioningScale,
    _axisRowPositioningScale,
    _axisColumnPositioningScale,
    _matrixID,
    _originalDataset,
    _rowLabels,
    _columnLabels,
  ...
  var _axisPositioningScaleCopy,
    _axisRowPositioningScaleCopy,
    _axisColumnPositioningScaleCopy;
  ...
  function chart(data, parentID) {
    ...
    _dissMatrix = normalize_Matrix(
      _dissMatrix,
      _matrixRowSize,
      _matrixColumnSize
    );
    _adjacencyMatrix = normalize_Matrix(
      _adjacencyMatrix,
      _matrixRowSize,
      _matrixColumnSize
    );
    ...
    _axisPositioningScale = d3.scale
      .ordinal()
      .domain(d3.range(_matrixSize))
      .rangeBands([0, _width]);
    _axisColumnPositioningScale = d3.scale
      .ordinal()
      .domain(d3.range(_matrixColumnSize))
      .rangeBands([0, _width]);
    _axisRowPositioningScale = d3.scale
      .ordinal()
      .domain(d3.range(_matrixRowSize))
      .rangeBands([0, _height]);

    _axisPositioningScaleCopy = d3.scale
      .ordinal()
      .domain(d3.range(_matrixSize))
      .rangeBands([0, _width]);
    _axisColumnPositioningScaleCopy = d3.scale
      .ordinal()
      .domain(d3.range(_matrixColumnSize))
      .rangeBands([0, _width]);
    _axisRowPositioningScaleCopy = d3.scale
      .ordinal()
      .domain(d3.range(_matrixRowSize))
      .rangeBands([0, _height]);
    ...
    svg
      ...
      .attr("width", 10)
      .attr("height", 10)
      .attr("xlink:href", "static/images/lock.svg")
      .attr("opacity", 0);
    // svg.append("line")
    //    .attr("stroke-width",2)
    ...
    row
      ...
      .style("fill", function (d, i) {
        if (
          _community_assignment_result &&
          true == AugmentedNodeTrix._communityDetection
        ) {
  ...
  function highlightDehighlightPath(src, dest, highlight) {
    if (highlight) {
      var path = d3.selectAll(
        ".bezierCurves.n" + src + ",.bezierCurves.n" + dest
      );
  ...
  chart.removeMultilayerPaths = function (index) {
    ...
    d3.selectAll(" .layer3.mat" + index)
      .attr("opacity", 0)
      .classed("bezierCurvesVisible", false)
      .classed("bezierCurvesInvisible", true);
  };
  chart.rerenderMultilayerPaths = function (index) {
    _showLayer3Edges = true;
    // d3.selectAll('#' + AugmentedNodeTrix._parentID +
    //    ' .mat'+index + '.layer3').attr("opacity",1);
    d3.selectAll(" .mat" + index + ".layer3")
      .attr("opacity", 1)
      .classed("bezierCurvesInvisible", false)
      .classed("bezierCurvesVisible", true);
  ...
  chart.deFocusNode = function (state) {
    ...
    if (_state == "freezed") {
      circle.style("fill", _stateColors.freezed).style("opacity", 0);
    ...
    if ("circle" == AugmentedNodeTrix.glyphType) {
      var cell = d3
        ...
        .attr("cx", function (d, i) {
          return (
            _axisPositioningScale.rangeBand() * i +
        ...
        .on("mouseover", function (d, colIndex) {
          ...
          d3.selectAll("#" + _matrixID + " .column text").classed(
            "active",
            function (d, i) {
              return i == colIndex;
            }
  ...
  chart.updateColorScale = function (color1, color2) {
    ...
    if ("HM" == color1 && "HM" == color2) {
      ...
      if (true == AugmentedNodeTrix._LABColorSpace) {
        ...
        _colorScale
          .domain([0, 1])
          .range([d3.lab(color1).brighter(), d3.lab(color2).brighter()]);
  ...
  chart.updateCombinedMatrix = function (alpha) {
    ...
    for (var i = 0; i < _matrixRowSize; i++)
      for (var j = 0; j < _matrixColumnSize; j++) {
        _matrix[i][j] =
          1 -
          ((1 - _dissMatrix[i][j]) * (1 - _alpha) +
            _alpha * (1 - _adjacencyMatrix[i][j]));
  ...
  chart.width = function () {
    return (
      d3.select("#" + _matrixID).attr("width") *
      AugmentedNodeTrix.globalScaleFactor
    );
  };
  chart.height = function () {
    return (
      d3.select("#" + _matrixID).attr("height") *
      AugmentedNodeTrix.globalScaleFactor
    );
  };
  ...
  chart.reCalculateSize = function () {
    ...
    _cellWidthScale =
      _cellWidthScaleOriginal * AugmentedNodeTrix.globalScaleFactor;
  ...
  chart.connectingPosition = function (authorID, connectionType) {
    ...
    if ("left" == connectionType) {
      ...
      var rowOffset = $(
        "svg" + "#" + _matrixID + " #Row" + currentAuthorPosition + " .cell"
      )
        .last()
        .offset();
      rowOffset.left = rowOffset.left - offSetChart.left;
      ...
      yposition =
        drawnMatrixTopLeftY +
        _axisColumnPositioningScaleCopy.rangeBand() * _matrixRowSize;
  ...
  function calculate_D(matrix, index) {
    ...
    for (var i = 0; i < index; i++) {
      ...
      for (var j = 0; j < index; j++) {
        ...
        else temp_distance = approx_dissimilarity(matrix[i], matrix[j]);
  ...
  chart.rectVAT = function () {
    ...
    rectVAT_matrix = normalize_Matrix(
      rectVAT_matrix,
      rectVAT_matrix.length,
      rectVAT_matrix[0].length
    );
    // console.log(rectVAT_matrix);
    ...
    if (min == max && 0 == min) {
      ...
      for (var i = 0; i < rowSize; i++) {
        for (var j = 0; j < colSize; j++) {
          ...
          } else matrix[i][j] = i == j ? 0 : (matrix[i][j] - min) / (max - min); //Why the hell should i do this ???
  ...
  function VAT1(matrix, matrixSize) {
      ...
      for (var i = 0; i < verticesChosen.length; ++i) {
        for (var j = 0; j < vertices.length; ++j) {
          if (
            ...
            matrix[verticesChosen[i]][vertices[j]] <= minimumDistance
          ) {
    ...
    for (var i = 0; i < matrixSize; i++) Matrix2[i] = new Array(matrixSize);
    // Exchange Columns
    for (var i = 0; i < matrixSize; ++i) {
      for (var j = 0; j < matrixSize; ++j) {
  ...
  chart.applyVAT = function () {
    ...
    while (0 != vertices.length) {
      ...
      for (var i = 0; i < verticesChosen.length; ++i) {
        for (var j = 0; j < vertices.length; ++j) {
          ...
          if (
            verticesChosen[i] != vertices[j] &&
            _matrix[verticesChosen[i]][vertices[j]] <= minimumDistance
          ) {
    ...
    for (var i = 0; i < _matrixSize; i++) Matrix2[i] = new Array(_matrixSize);
    // Exchange Columns
  ...
  chart.applyCrossingReduction = function () {
    ...
    var perms = reorder.barycenter_order(graph);
    var improved = reorder.adjacent_exchange(graph, perms[0], perms[1]);
  ...
  chart.applyCLUSION = function () {
    ...
    if (_clusteringDone == true) {
      ...
      indices.sort(function (a, b) {
        return _clusterVector[a] < _clusterVector[b]
          ? -1
          : _clusterVector[a] > _clusterVector[b]
          ? 1
          : 0;
      });
      console.log(_clusterVector);
  ...
  chart.applyCommunityDetectionLeuven = function () {
    ...
    // Exchange Columns back to original matrix
    for (var i = 0; i < _matrixSize; ++i)
      for (var j = 0; j < _matrixSize; ++j) _matrix[i][j] = Matrix2[i][j];
    chart.renderUpdatedMatrix();
    return chart;
  };
  ...
  chart.normalizedGraphLaplacianMatrix = function (
    degreeMatrix,
    adjacencyMatrix
  ) {
    ...
    var normLaplaceMatrix = math.multiply(
      math.multiply(
        dInverseSquareRoot,
        math.add(degreeMatrixForm, negativeAdjacency)
      ),
      dInverseSquareRoot
    );
  ...
  chart.applyModularity = function () {
    ...
    var m = 0;
    for (var i = 0; i < _matrixRowSize; i++) degreeList[i] = 0;
    ...
    for (var i = 0; i < _matrixRowSize; i++) m = m + degreeList[i];
    ...
    for (var i = 0; i < _matrixRowSize; i++) {
      for (var j = 0; j < _matrixColumnSize; j++) {
        _matrix[i][j] =
          _matrix[i][j] - (degreeList[i] * degreeList[j]) / (2 * m);
      }
    }
  ...
  chart.applyPower2 = function () {
    ...
    _matrix = normalize_Matrix_Standard(
      _matrix,
      _matrixRowSize,
      _matrixColumnSize
    );
  ...
  chart.linkPrediction = function (operation, index) {
    ...
    idsToLoad = [];
    for (var i = 0; i < _rowLabels.length; i++) {
      if (!isNaN(_rowLabels[i].id)) idsToLoad.push(_rowLabels[i].id);
    }
    // console.log(_rowLabels);
        ...
        for (var i = 0; i < _matrixRowSize; i++) {
          for (var j = 0; j < _matrixColumnSize; j++) {
            if (recdMatrix[i][j] == -1) {
              ...
              tempMatrix2[i][j] = -2;
            } else if (recdMatrix[i][j] == 0) tempMatrix2[i][j] = 1;
            else {
              tempMatrix2[i][j] = 1 - recdMatrix[i][j];
            }
          }
  ...
  chart.applyClustering = function (dimensions, clusters, type) {
    ...
    if (_clusterdrift["nodes"].length == 0) {
      for (var i = 0; i < _columnLabels.length - 1; i++) {
        temp = {
          name: _columnLabels[i].name,
          id: _columnLabels[i].id,
          connects: [],
        };
  ...
  chart.updateValueFiltering = function (beta) {
    ...
    for (var i = 0; i < _matrixRowSize; i++) {
      for (var j = 0; j < _matrixColumnSize; j++) {
        if (tempMatrix[i][j] >= _beta) tempMatrix[i][j] = 1;
      }
    }
    _matrix = tempMatrix;
    // _matrix = normalize_Matrix(_matrix, _matrixRowSize, _matrixColumnSize);
    ...
    if ("circle" == AugmentedNodeTrix.glyphType) {
      ...
      var cell = d3
        ...
        .style("fill", function (d, i) {
          ...
          if (data[i] == -1) return "red";
          else if (data[i] == -2) return "green";
          else if (data[i] > 1) return _colorScale(1);
          else return _colorScale(data[i]);
  ...
  chart.drawClusteringGraph = function () {
    ...
    for (var i = 0; i < numberOFNodes; i++) nodeGroupBelonging[i] = -1;
    ...
    for (var i = 0; i < d.length + d[0].length; i++) {
      temp = {
        name: i < d.length ? "cluster " + i : _columnLabels[i - d.length].name,
        cluster: i < d.length ? i : nodeGroupBelonging[i - numberOfClusters],
        cluster_centre: i < d.length ? true : false,
        radius: i < d.length ? 10 : 5,
        index: i,
      };
      nodes1.push(temp);
    }
    ...
    var svg = d3
      ...
      .on("contextmenu", function (data, index) {
        ...
        $("#" + id)
          .clone()
          .appendTo("#collectedview");

        var svgs = d3
          .selectAll("#collectedview svg")
          .on("contextmenu", function (data, index) {
            d3.event.preventDefault();
            this.remove();
          });
    ...
    function tick(e) {
      link
        .attr("x1", function (d) {
          return nodes1[d.source].x;
        })
        .attr("y1", function (d) {
          return nodes1[d.source].y;
        })
        .attr("x2", function (d) {
          return nodes1[d.target].x;
        })
        .attr("y2", function (d) {
          return nodes1[d.target].y;
        });
